 ...
 *
 * WooCommerce is gone, so the cart lives in localStorage and the order is
 * emailed by /api/order. The markup below is WooCommerce's own classic cart /
 * checkout / order-received markup, because the theme's stylesheets already
 * style those classes -- nothing new is designed here.
 *
...
(function () {
  ...
  var STORE_KEY = 'ttp_cart';
  var ORDER_KEY = 'ttp_last_order';
  var catalogue = null;
  ...
  function renderCart() {
    ...
    var filled = document.getElementById('ttp-cart-filled');
    var ls = lines();
  ...
  function renderCheckout() {
    var root = document.getElementById('ttp-checkout');
    if (!root) return;
    var ls = lines();
    ...
    if (!ls.length) {
      root.innerHTML = '<div class="woocommerce"><div class="woocommerce-notices-wrapper">' +
        ...
        '<p><a class="button wc-backward" href="/shop/">Return to shop</a></p></div>';
      return;
    }

    root.innerHTML = '<div class="woocommerce">' +
      '<div class="woocommerce-notices-wrapper" id="ttp-checkout-notices"></div>' +
      ...
      '<form name="checkout" method="post" novalidate class="checkout woocommerce-checkout" id="ttp-checkout-form">' +
    ...
    document.getElementById('ttp-checkout-form')
  ...
  function placeOrder(event) {
    ...
    var notices = document.getElementById('ttp-checkout-notices');
    notices.innerHTML = '';
  ...
  function renderOrderReceived() {
    var root = document.getElementById('ttp-order-received');
    if (!root) return;
    var order;
  ...
  function wire() {
    ...
    document.addEventListener('ttp:cart-changed', function () {
      renderCart();
      renderCheckout();
    });
  }
