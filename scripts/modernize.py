...
MENU_ICONS_JS = ROOT / "src" / "lib" / "menu-icons.js"
...
ICONS = {
    ...
    "tube": svg(
        f'<ellipse cx="12" cy="6" rx="6" ry="2.6"/>',
        p("M6 6v12c0 1.44 2.69 2.6 6 2.6s6-1.16 6-2.6V6"),
    ),
    "roll": svg(
        f'<ellipse cx="12" cy="6.5" rx="7" ry="3"/>',
        p("M5 6.5v11c0 1.66 3.13 3 7 3s7-1.34 7-3v-11"),
        f'<ellipse cx="12" cy="6.5" rx="2.2" ry="1"/>',
    ),
    "mailer": svg(
        f'<rect x="2.5" y="8" width="19" height="8" rx="4"/>',
        p("M7 8v8"), p("M17 8v8"),
    ),
    "squeeze": svg(
        p("M9.6 9.5 8.5 21h7l-1.1-11.5"),
        p("M9.6 9.5a3 3 0 0 1 .4-1.5V6h4v2a3 3 0 0 1 .4 1.5z"),
        p("M10.5 3h3v3h-3z"),
    ),
    "utensils": svg(
        p("M3 2v7c0 1.1.9 2 2 2h1a2 2 0 0 0 2-2V2"),
        p("M7 2v20"),
...
def add_labels(html):
    ...
    def select_repl(m):
        nonlocal added
        # the option text carries the source's stray double spaces
        raw = " ".join(m.group(5).split())
        text = LABEL_TEXT.get(raw, raw)
        if not text:
            return m.group(0)
        added += 1
        return (
...
# ── icons in the header and drawer dropdowns ────────────────────────────
# Every dropdown entry is a product page, so each link gets the icon for what
# the tube is actually for. Anything unmapped falls back to a plain cylinder,
# which is never wrong for this catalogue.
MENU_ICONS = {
    "kraft-paper-candle-tubes": "candle",
    "white-paper-candle-tubes": "candle",
    "black-paper-candle-tubes": "candle",
    "candle-tube-packaging": "candle",
    "paper-lip-balm-tubes": "lipstick",
    "paper-lipstick-tubes": "lipstick",
    "white-lipstick-tubes": "lipstick",
    "paper-towel-tubes": "roll",
    "cardboard-toilet-paper": "roll",
    "wrapping-paper-tubes": "gift",
    "square-paper-tubes": "package",
    "tea-paper-tubes": "cup",
    "cosmetic-tubes": "bottle",
    "skincare-tubes": "bottle",
    "deodorant-paper-tubes": "bottle",
    "poster-mailing-tubes": "mailer",
    "cylinder-mailing-tubes": "mailer",
    "kraft-mailing-tubes": "truck",
    "custom-shipping-tubes": "truck",
    "luxury-tube-packaging": "sparkles",
    "tube-food-packaging": "utensils",
    "empty-lipgloss-tubes": "squeeze",
    "empty-lotion-tubes": "squeeze",
    "lotion-tubes": "squeeze",
    "plastic-tube-packaging": "squeeze",
    "industrial-cardboard-tubes": "factory",
}

def write_menu_icons():
    """Emit the menu icon set as a module the chrome renderer applies.

    `chrome.json` must not be edited to add these. `region()` in
    src/lib/chrome.js splits the header and drawer on tags and replaces tokens
    **by index** from each page's `chromeDiff`, so inserting anything into the
    stored markup shifts every later index — the first attempt at this pushed
    the site logo into a menu link. Injecting after the diff has been applied
    leaves the stored markup, and the indices, untouched.
    """
    entries = {}
    for slug, name in sorted(MENU_ICONS.items()):
        entries[slug] = ICONS[name]
    body = ",\n".join(
        f"  {json.dumps(k)}: {json.dumps(v)}" for k, v in entries.items()
    )
    module = f'''/**
 * Product icons for the header and drawer dropdown links.
 *
 * GENERATED by scripts/modernize.py -- do not edit by hand.
 *
 * These are applied by `region()` in chrome.js, after the per-page
 * `chromeDiff` has been merged: that merge is index-based, so the icons can
 * never be baked into src/data/chrome.json without shifting every token after
 * them.
 */
const ICONS = {{
{body}
...
const FALLBACK = {json.dumps(ICONS["tube"])};

const PANEL = /<ul class="sub-menu">[\\s\\S]*?<\\/ul>/g;
const LINK = /<a href="[^"]*\\/product\\/([a-z0-9-]+)\\/"[^>]*>/g;

/** Put a product icon in front of every dropdown link. */
export function withMenuIcons(html) {{
  return html.replace(PANEL, (panel) =>
    panel.replace(LINK, (tag, slug) =>
      `${{tag}}<span class="tpm-mi" aria-hidden="true">${{ICONS[slug] || FALLBACK}}</span>`
    )
  );
}}
'''
    MENU_ICONS_JS.write_text(module)
    return len(entries)
...
def main():
    ...
    print("menu icons written            :", icons, "slugs + fallback")
    if all_misses:
